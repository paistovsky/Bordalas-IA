...
def betting_pool_is_economic(
    content,
) -> bool:
    """
    Una quiniela solo mueve saldo de liga si de verdad hubo
    participacion y reparto.

    Los "credits" del pool son moneda propia de Biwenger, no el
    saldo de la liga, asi que no cuentan. Pero si algun dia
    aparecen premios o respuestas, este evento deja de ser
    decorativo y hay que mirarlo: por eso NO se mete en la lista
    blanca sin condiciones.
    """
...
def build_rival_intelligence(
    *,
    ...
    # event_id distintos. Comprobado el 15/08/2026: el fichaje de
    # Yamal por 24.897.600 aparece en los eventos 01172994d5b4 y
    # 5748a8232cb8, misma fecha y mismo importe.
    #
    # Deduplicar por event_id no sirve -son distintos-, asi que
    # la huella se hace con el contenido economico. Sin esto el
    # ledger contaba tres compras como seis y descuadraba el
    # saldo en 28.097.432 EUR, justo el doble de los gastos.
    seen_operations = set()
    ...
    def already_counted(
        ...
        huella = (
            ...
            safe_int(player_id),
            safe_int(amount),
            safe_int(counterparty),
            safe_int(when),
        )

        if huella in seen_operations:
            return True

        seen_operations.add(huella)
        return False
    ...
    for event in sorted(
        ...
        if event_type == "market":
            ...
            == "bettingPool"
            and
            not betting_pool_is_economic(
                content
            )
        ):

            # Quiniela sin premios ni respuestas: decorativa.
            # Si algun dia trae reparto, cae al else y se marca
            # como desconocida para que salte la revision.
            pass
