...
export default function KpiStrip({ data }) {
  ...
  const totalValue = Number(lineup.total_value || 0) > 0
    ? Number(lineup.total_value)
    : (lineup.players || []).reduce(
        (sum, player) => sum + Number(player.price || 0),
        0
      );

  const mode = summary.phase === "NORMAL" ? "SIN URGENCIA" : "ATENTO";
  ...
  return (
    <div className="kpi-strip">
      ...
      <Kpi icon={BarChart3} label="VALOR XI" value={formatMoney(totalValue)} helper="Titulares" tone="neutral" />
      ...
      <Kpi icon={ShieldCheck} label="ESTADO" value={mode} helper="Estrategia protegida" tone="success" />
    </div>
  );
}
